        ...
        with open(log_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(parts) >= 5:
                    ...
                    user_log = parts[-1].replace('utilisateur:', '').strip()
                    if (start_log == log_data['start'] and end_log == log_data['end'] and app_log == app and user_log == USERNAME):
                        already_exists = True
                        break
    # Envoyer au backend seulement si le username existe
    if USERNAME:
        try:
        ...
        print("Pas de username trouvé, activity_logs non envoyés.")
    if not already_exists:
        # Écriture complète avec la durée
        full_text = f"{log_data['start']}|{log_data['end']}| {app}| {duration} secondes| utilisateur: {USERNAME}"
